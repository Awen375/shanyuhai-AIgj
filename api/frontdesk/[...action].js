...
export default async function handler(req, res) {
    try {
        // 纯字符串方式解析路径，避免 URL 对象异常
        const pathOnly = req.url.split('?')[0];                     // 例如 /api/frontdesk/notifications
        let rawAction = pathOnly.replace('/api/frontdesk/', '');   // 去掉前缀
        if (rawAction === '/api/frontdesk') rawAction = '';        // 处理无子路径的情况
        const action = rawAction || 'main';                        // 默认 main
